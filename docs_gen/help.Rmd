...
<tr><td>METHOD <td>GET
<tr><td>PARAMETERS <td>
- **admin** _optional_ 0 or 1: if set to 1, open a priority session. This is identical to the `--admin` flag for the `iquery` client (see [SciDB Documentation](https://paradigm4.atlassian.net/wiki/spaces/scidb) for details). The default value is 0.
...
- **release** _optional_ 0 or 1: if 1 then release_session as soon as query completes. The default value is 0 if not specified (see additional notes below).
